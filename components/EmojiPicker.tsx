...
export function EmojiPicker({ visible, onClose, onSelect }: EmojiPickerProps) {
	...
	const handlePageSelected = (
		event: NativeSyntheticEvent<PagerViewOnPageSelectedEvent>,
	) => {
	...
	const renderIcon = ({ item }: { item: string }) => (
		...
		>
			<MaterialIcons name={item as string} size={28} color={textColor} />
		</TouchableOpacity>
	)
				...
				>
					...
					>
						{categories.map(category => (
							<View key={category}>{renderPage(category)}</View>
						))}
					</PagerView>
					...
					<TouchableOpacity
						style={[
							...
							{
								backgroundColor:
									dayColor === textColor ? "#f0f0f0" : inputBackgroundColor,
							},
						]}
						onPress={onClose}
					>
