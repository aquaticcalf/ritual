...
const CreateHabitScreen = () => {
	...
	const [reminderTime, setReminderTime] = useState<Date | undefined>(
		habitData?.reminderTime ? new Date(habitData.reminderTime) : undefined,
	)
	...
	}, [habitData, navigation.setOptions])

	useEffect(() => {
		if (reminder && !reminderTime && !isWeb) {
			setReminderTime(new Date())
		}
	}, [reminder, isWeb, reminderTime])
	...
	const validateForm = () => {
		...
		if (
			initialStreak < 0 ||
			!Number.isInteger(Number(initialStreak)) ||
			initialStreak > 10
		) {
	...
	const handleEditHabit = async () => {
		...
		const frequencyChanged =
			JSON.stringify(habitData.frequency) !== JSON.stringify(frequency)
		...
		const updatedHabit = {
			...
			...(frequencyChanged && {
				frequencyUpdatedDate: new Date().toISOString(),
			}),
		}
		...
		try {
			const existingHabits = await AsyncStorage.getItem("habits")
			const habits = existingHabits ? JSON.parse(existingHabits) : []
			const habitIndex = habits.findIndex((h: Habit) => h.id === habitData.id)
	...
	return (
		<ThemedView style={[styles.container, { backgroundColor }]}>
			<ScrollView contentContainerStyle={styles.scrollContainer}>
				...
				<ThemedText style={[styles.subLabel, { color: secondaryTextColor }]}>
