...
import { checkAndUpdateStreak, formatDate } from "@/lib/utils"
...
const HomeScreen = () => {
	...
	const handleMarkAsDone = async ({
		...
		const currentHabitState = validationResult.updatedHabit
		const needsSaveAfterCheck =
			JSON.stringify(currentHabitState) !== JSON.stringify(habitToCheck)
		...
		if (currentHabitState.lastDone === formattedToday) {
			const unmarkHabit = () => {
				...
				const unmarkedHabit = { ...currentHabitState }

				const streakBeforeDecrement = unmarkedHabit.currentStreak
				unmarkedHabit.currentStreak = Math.max(
					0,
					unmarkedHabit.currentStreak - 1,
				)

				if (
					streakBeforeDecrement > 0 &&
					streakBeforeDecrement % 5 === 0 &&
					unmarkedHabit.currentStreak === streakBeforeDecrement - 1
				) {
					unmarkedHabit.freezesAvailable = Math.max(
						0,
						(unmarkedHabit.freezesAvailable || 0) - 1,
					)
				}

				if (unmarkedHabit.prevBestStreak !== undefined) {
					unmarkedHabit.bestStreak = Math.max(
						unmarkedHabit.prevBestStreak,
						unmarkedHabit.currentStreak,
					)
				}

				unmarkedHabit.heatMap = (unmarkedHabit.heatMap || []).filter(
					entry => entry.date !== formattedToday,
				)
				const latestHeatmapDate =
					unmarkedHabit.heatMap.length > 0
						? unmarkedHabit.heatMap[unmarkedHabit.heatMap.length - 1].date
						: ""
				unmarkedHabit.lastDone = latestHeatmapDate

				// Create the final habits array
				const finalHabits = habits.map((h, index) =>
					index === habitIndex ? unmarkedHabit : h,
				)
		...
		if (currentHabitState.frequency.includes(todayDayCode)) {
			...
			const markedHabit = { ...currentHabitState }
			...
			// Update heatmap
			const heatMapEntryExists = (markedHabit.heatMap || []).some(
				entry => entry.date === formattedToday,
			)
			if (!heatMapEntryExists) {
				markedHabit.heatMap = [
					...(markedHabit.heatMap || []),
					{ day: todayDayCode, date: formattedToday },
				]
			...
			markedHabit.freezeMap = (markedHabit.freezeMap || []).filter(
				entry => entry.date !== formattedToday,
			)
			...
			// Create the final habits array
			const finalHabits = habits.map((h, index) =>
				index === habitIndex ? markedHabit : h,
			)
			...
			if (needsSaveAfterCheck) {
				const finalHabits = habits.map((h, index) =>
					index === habitIndex ? currentHabitState : h,
				)
				await AsyncStorage.setItem("habits", JSON.stringify(finalHabits))
	...
	const fetchAndCheckHabits = useCallback(async () => {
		try {
			const existingHabitsJson = await AsyncStorage.getItem("habits")
			let habitsData: Habit[] = existingHabitsJson
			...
			habitsData = habitsData.map(h => ({
				...
				freezesAvailable:
					h.freezesAvailable === undefined ? 0 : h.freezesAvailable,
			}))
			...
			const processedHabitsPromises = habitsData.map(async habit => {
				...
				if (result.freezesConsumed > 0) {
					...
					if (attemptedToConsume > 0) {
						// Some freezes were used before breaking
						freezeMessages.push({
							type: "warn",
							text1: "Streak Lost",
						...
						// No freezes were available at all
						freezeMessages.push({
							type: "warn",
							text1: "Streak Lost",
			...
			const uniqueMessages = new Map<
				string,
				{ type: "info" | "warn"; text1: string; text2: string }
			>()
			freezeMessages.forEach(msg =>
				uniqueMessages.set(JSON.stringify(msg), msg),
			)
	...
	useEffect(() => {
		fetchAndCheckHabits()
	}, [fetchAndCheckHabits]) // Dependency array ensures it runs on mount
		...
		>
			...
			{isLoading ? (
				...
				>
					...
					<ThemedText style={{ marginTop: 10, color: textColor }}>
						Loading habits...
					</ThemedText>
				</View>
			) : (
				...
				>
					{habits.length === 0 ? (
						...
						>
							<LogoIcon width={48} height={48} style={{ marginBottom: 15 }} />
							<ThemedText
								style={{
									color: textColor,
									fontSize: 20,
							...
							>
								Transform your habits into daily rituals. Track your progress
								with heatmaps, build streaks, and never break the chain.
							</ThemedText>
							<ThemedText
								style={{
									color: secondaryTextColor,
						...
						<FlatList
							...
							renderItem={({ item }) => {
								...
								const lastHeatMapDate =
									item.heatMap?.length > 0
										? item.heatMap[item.heatMap.length - 1].date
										: null
								const lastFreezeMapDate =
									item.freezeMap?.length > 0
										? item.freezeMap[item.freezeMap.length - 1].date
										: null

								if (lastHeatMapDate && lastFreezeMapDate) {
									const lastHeatDate = new Date(
										Number.parseInt(lastHeatMapDate.split("/")[2]),
										Number.parseInt(lastHeatMapDate.split("/")[1]) - 1,
										Number.parseInt(lastHeatMapDate.split("/")[0]),
									)
									const lastFreezeDate = new Date(
										Number.parseInt(lastFreezeMapDate.split("/")[2]),
										Number.parseInt(lastFreezeMapDate.split("/")[1]) - 1,
										Number.parseInt(lastFreezeMapDate.split("/")[0]),
									)

									// If last heat map entry is not newer than last freeze AND streak isn't 0
									if (
										!(lastHeatDate > lastFreezeDate) &&
													...
													>
														...
														>
															{item.frequency.length === 7
																? "Every day"
																: `${item.frequency.length} days a week`}
														</ThemedText>
													</View>
													<View
														style={{
