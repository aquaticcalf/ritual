...
export const generateHistoricalHeatMap = (
	streak: number,
	frequency: string[],
): {
	heatMap: Cell[]
...
function getStartAndEndDateOfMissedScheduledDates(
	habit: Habit,
	today: Date,
): {
	startDate: Date | null
	...
	if (habit.freezeMap && habit.freezeMap.length > 0) {
		...
		const latestFreezeDateStr = habit.freezeMap.reduce((latest, current) => {
			...
			const latestDate = new Date(
				Number.parseInt(latestParts[2]),
				Number.parseInt(latestParts[1]) - 1,
				Number.parseInt(latestParts[0]),
			)
			const currentDate = new Date(
				Number.parseInt(currentParts[2]),
				Number.parseInt(currentParts[1]) - 1,
				Number.parseInt(currentParts[0]),
			)
		...
		if (latestFreezeDateStr) {
			const latestFreezeDate = new Date(
				Number.parseInt(latestFreezeDateStr.split("/")[2]),
				Number.parseInt(latestFreezeDateStr.split("/")[1]) - 1,
				Number.parseInt(latestFreezeDateStr.split("/")[0]),
			)
			const lastDoneDate = habit.lastDone
				? new Date(
						Number.parseInt(habit.lastDone.split("/")[2]),
						Number.parseInt(habit.lastDone.split("/")[1]) - 1,
						Number.parseInt(habit.lastDone.split("/")[0]),
					)
				: null
	...
	const frequencyUpdateDate = habit.frequencyUpdatedDate
		? new Date(habit.frequencyUpdatedDate)
		: null
	...
	if (lastCompletionOrFreezeDateStr) {
		...
		const effectiveLastDate = new Date(
...
export const checkAndUpdateStreak = (
	...
	const updatedHabit = {
		...
		freezesAvailable:
			habit.freezesAvailable === undefined ? 0 : habit.freezesAvailable,
	}
	...
	const { startDate, endDate } = getStartAndEndDateOfMissedScheduledDates(
		updatedHabit,
		today,
	)
